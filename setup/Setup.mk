...
LIBPRI_ = $(patsubst %,$(OBJTGT)/usr/lib/gear/%,$(LIBPRI))
CFG_ = $(patsubst %,$(OBJTGT)/etc/gear/%,$(CFG))
NGX_ = $(patsubst %,$(OBJTGT)/etc/nginx/sites-enabled/%,$(NGX))
HTML_ = $(patsubst %,$(OBJTGT)/var/gear/html/%,$(HTML))
DATA_ = $(patsubst %,$(OBJTGT)/var/gear/data/%,$(DATA))
SQL_ = $(patsubst %,$(OBJTGT)/var/gear/sql/%,$(SQL))
...
$(LIBPRI_): $(OBJTGT)/usr/lib/gear/%: %; @echo "copy: $< => $@"; mkdir -p $(dir $@); cp -f $< $@
$(CFG_): $(OBJTGT)/etc/gear/%: %; @echo "copy: $< => $@"; mkdir -p $(dir $@); cp -f $< $@
$(NGX_): $(OBJTGT)/etc/nginx/sites-enabled/%: %; @echo "copy: $< => $@"; mkdir -p $(dir $@); cp -f $< $@
$(SQL_): $(OBJTGT)/var/gear/sql/%: %; @echo "copy: $< => $@"; mkdir -p $(dir $@); cp -f $< $@
$(HTML_): $(OBJTGT)/var/gear/html/%: %; @echo "copy: $< => $@"; mkdir -p $(dir $@); cp -f $< $@
$(DATA_): $(OBJTGT)/var/gear/data/%: %; @echo "copy: $< => $@"; mkdir -p $(dir $@); cp -f $< $@
